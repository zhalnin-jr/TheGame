...
internal class HealerUnit : Unit
{
    ...
    public void HealFirstUnitWithChance(List<Unit> units)
    {
        ...
        if (chance <= 25)
        {
            ...
            if (units.Count > 0)
            {
                Unit firstUnit = units[0]; // Получаем первого юнита из списка
                ...
                if (firstUnit is IHealableUnit healableUnit)
                {
                    ...
                    FrontManager.GetInstance().Printer($"Лечитель {Name} исцелил юнита {firstUnit.Name}.");
                }
                else
                {
                    FrontManager.GetInstance().Printer($"Юнит {firstUnit.Name} не может быть исцелен.");
                }
            }
        }
    }
